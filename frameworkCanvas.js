    ...
    moverComTeclado: function (el) {
        for (let a of arco) {
            if (a.getAttribute("interacao")) {
                ...
                if (el.key === "ArrowLeft") this.x -= this.velocidade;
                if (el.key === "ArrowRight") this.x += this.velocidade;
            };
        }
    }
}
...
function colisao() {
    let vetor = document.querySelectorAll("[colisao='true']");
